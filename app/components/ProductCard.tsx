...
const ProductCard = ({
  ...
  const addToCart = () => {
    ...
    const selectedSizeObj = selectedVariation.sizes.find(
      ...
      <div className="bg-primaryLight text-primary flex max-md:min-w-[40vw] flex-col rounded-2xl overflow-hidden shadow-md cursor-pointer hover:scale-[1.01] transition-all">
        ...
        <div className="p-4 flex-1 gap-3 flex flex-col">
          <div className="flex-1">
            <h5
              className={`text-sm md:text-base xl:text-base sm:text-base ${Berkishire.className} font-bold  line-clamp-1`}
            >
              {product.title}
            </h5>
            <div className="flex items-center flex-wrap">
              <div>
                {product?.comparedPrice > 0 && (
                  <h6 className="text-[10px] text-gray-400 sm:text-xs line-through font-bold">
                    {product.comparedPrice} EGP
                  </h6>
                )}
                <h6 className="text-sm sm:text-base font-bold">
                  {product.price.local} EGP
                </h6>
              </div>
              <button
                id="addToWishList"
