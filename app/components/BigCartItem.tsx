...
const BigCartItem = ({ cartItem }: { cartItem: CartItem }) => {
  ...
  return (
    <div className="flex w-full px-2 py-1 border-2 rounded-md border-primary items-start text-primary mr-28">
      <span
        onClick={() => toggleRemoveModal((prevState) => !prevState)}
        className="hover:cursor-pointer mr-10 md:mr-20 lg:mr-24 hover:rotate-180 transition duration-700"
      >
        x
      </span>
      ...
      <div className="flex  pb-2 w-full justify-between items-start text-primary">
        ...
        <p className="text-[12px] lg:text-lg">
          <span
            ...
            onClick={() => handleQuantity("-", cartItem.id)}
          >
            &lt;{" "}
          </span>
          {quantity}
        ...
        {modalAppear && (
          <div className="absolute z-20 top-[40%] sm:top-[30%] sm:left-[30%] bg-backgroundColor left-[10%] w-[80vw] sm:w-[40vw] px-2 h-auto min-h-[20vh]">
            <span
              onClick={() => toggleRemoveModal(false)}
              className="hover:cursor-pointer hover:rotate-180 transition-transform inline-block duration-700"
            >
              x
            </span>
            <h1 className="py-6 text-center text-sm">
