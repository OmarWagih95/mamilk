...
const ContactPage = () => {
  ...
  return (
    <>     
<div className='min-h-screen pt-[110px] pb-[20px] text-primary bg-primaryLight'>
<div className="max-w-6xl max-lg:max-w-3xl rounded-2xl mx-auto bg-backgroundColor shadow-[0_4px_6px_rgba(0,0,0,0.1),0_-4px_6px_rgba(0,0,0,0.1),6px_0_6px_rgba(0,0,0,0.1),-6px_0_6px_rgba(0,0,0,0.1)]">
...
<p className="text-sm text-primary mt-4 leading-relaxed">
  Whether you have a question, a story to share, or an idea you’re excited about, we’re all ears. 
  At Mamilk, we believe in building real connections with our community — that means listening, 
  learning, and growing together. <br /><br />
  Your thoughts matter to us, and no message is too small. Whether you're a new parent with a curious mind, 
  a loyal customer with feedback, or someone who just stumbled upon us and wants to say hi — we’re here for it. <br /><br />
                ...
                <div className=" p-6 rounded-lg">
                    ...
                    className="mt-8 space-y-3">
                        ...
                        <input value={state.name} onChange={(e)=>handleChange('name',e.target.value)} type='text'  placeholder='Name'
                            className="w-full bg-babyBlue py-3 px-4 rounded-2xl text-gray-800 text-sm outline-primary" />
                                                {errors.name && <p className='text-xs text-red-500'>{errors.name}</p>}
                        </div>

                        <div className='flex flex-col gap-1'>
                        <input value={state.email} onChange={(e)=>handleChange('email',e.target.value)} type='email' placeholder='Email'
                            className="w-full bg-babyBlue py-3 rounded-2xl px-4 text-gray-800 text-sm outline-primary" />
...
<div className='flex flex-col gap-1'>
                        ...
                        <textarea value={state.message} onChange={(e)=>handleChange('message',e.target.value)} placeholder='Message' rows={6}
                            className="w-full bg-babyBlue px-4 rounded-2xl text-gray-800 text-sm pt-3 outline-primary"></textarea>
                                                                            {errors.message && <p className='text-xs text-red-500'>{errors.message}</p>}
                            </div>

                       {loading?<button disabled type='submit'
                       
                       className="text-white bg-gray-400 rounded-2xl tracking-wide  text-sm px-4 py-3 flex items-center justify-center w-full !mt-6">
    
                       Loading ...
                   </button> :<button type='submit'
                       
                            className="text-white bg-accent rounded-2xl hover:bg-primary   tracking-wide text-sm px-4 py-3 flex items-center justify-center w-full !mt-6">
